...
class Corrp():
	...
	def __init__(self,flag,pomdpfile='program.pomdp'):
		#self.obj=Reason()
		print 'flag is ',flag
		self.flag=flag
		self.obj = None
	...
	def init_belief(self):        #remember later to change it to override method
		...
		if obs_rand =='currenttime=':
			int_prob = float( self.obj.query('reason.plog',self.decision,self.time,self.location,obs_rand+self.time) )
		elif obs_rand =='atlocation=':
			int_prob = float( self.obj.query('reason.plog',self.decision,self.time,self.location,obs_rand+self.location) )
		self.obj.delete('reason.plog')
		print int_prob
		if self.flag:
			print 'hi'
			self.lstm()
			int_prob = 0.75*self.suplearn + 0.25*int_prob
	...
	def lstm(self):
		...
		if self.decision == 'interested':
			self.suplearn = random.choice(pred_int)

		else:
			self.suplearn = random.choice(pred_not_int)
